...
var checkLeopard = function(design, board, player, pos, dir, leopard) {
  var p = design.navigate(player, pos, dir);
  if (p === null) return false;
  p = design.navigate(player, p, dir);
  if (p === null) return false;
  piece = board.getPiece(p);
  if (piece == null) return false;
  return (piece.player != player) && (piece.type == leopard);
}
...
Dagaz.Model.CheckInvariants = function(board) {
  ...
  var leopard = design.getPieceType("Leopard");
  ...
  var ne = design.getDirection("ne"); var se = design.getDirection("se");
      _.each(board.moves, function(move) {
          ...
          if ((pos === null) ||
              ...
              checkLeopard(design, b, board.player, pos, nw, leopard)||
              checkLeopard(design, b, board.player, pos, sw, leopard)||
              checkLeopard(design, b, board.player, pos, ne, leopard)||
              checkLeopard(design, b, board.player, pos, se, leopard)) {
              move.failed = true;
          }
      });
