...
function FormatReserve(square) {
    ...
    return letters[square & 7] + (Dagaz.Model.HEIGHT - (square >> 3));
}
...
function Mobility(color) {
    ...
    // Copper mobility
    mob = -2;
    pieceIdx = (color | pieceCopper) << Dagaz.AI.COUNTER_SIZE;
    from = Dagaz.AI.g_pieceList[pieceIdx++];
    while (from != 0) {
        if (color == Dagaz.AI.colorWhite) {
...
function DropMobility(piece, from) {
    ...
    if ((piece & Dagaz.AI.TYPE_MASK) == pieceCopper) {
        if (color == Dagaz.AI.colorWhite) {
            mob += mobUnit[Dagaz.AI.g_board[from - 17]];
            mob += mobUnit[Dagaz.AI.g_board[from - 15]];
        } else {
...
Dagaz.AI.ResetGame = function() {
  ...
  pieceSquareAdj[pieceCopper]  = MakeTable(Dagaz.AI.pieceAdj[pieceCopper]);
...
function InitializeEval() {
    ...
    for (var i = 0; i < 2; i++) {
        ...
        g_mobUnit[i][enemy  | pieceCopper] = 3;
        ...
        g_mobUnit[i][friend | pieceCopper] = 0;
            ...
            if (c >= '0' && c <= '9') {
                ...
                switch (c) {
                    case 'p':
                        ...
                        break;
                    case 'w':
                        piece  = piecePawn | enemy;
                        break;
                    case 'n':
                        piece |= pieceKnight;
                        break;
                    case 'h':
                        piece  = pieceKnight | enemy;
                        break;
                    case 'c':
                        piece |= pieceCopper;
                        break;
...
function GetSlot() {
    for (var i = 0; i < RESERVE_SIZE; i+=2) {
        if (g_reserve[i] == 0) return i;
    }
    return null;
}
...
Dagaz.AI.GenerateAllMoves = function(moveStack) {
    ...
    // Copper quiet moves
    pieceIdx = (Dagaz.AI.g_toMove | pieceCopper) << Dagaz.AI.COUNTER_SIZE;
    from = Dagaz.AI.g_pieceList[pieceIdx++];
    while (from != 0) {
        to = from + (inc + 1); 
        if (Dagaz.AI.g_board[to] == 0) moveStack[moveStack.length] = GenerateMove(from, to, moveflagPromotion);
