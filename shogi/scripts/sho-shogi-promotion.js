...
Dagaz.Model.CheckInvariants = function(board) {
  ...
  if (promote.length == 0) {
      promote[ design.getPieceType("Silver") ] = design.getPieceType("Gold");
      promote[ design.getPieceType("Knight") ] = design.getPieceType("Gold");
      promote[ design.getPieceType("Lance") ]  = design.getPieceType("Gold");
      promote[ design.getPieceType("Pawn") ]   = design.getPieceType("Gold");
      promote[ design.getPieceType("Bishop") ] = design.getPieceType("BishopP");
      promote[ design.getPieceType("Rook") ]   = design.getPieceType("RookP");
      promote[ design.getPieceType("DrunkenElephant") ] = design.getPieceType("King");
  }
  _.each(board.moves, function(move) {
      if (move.isSimpleMove()) {
          var pos    = move.actions[0][0][0];
          var target = move.actions[0][1][0];
          if (design.inZone(0, board.player, pos) || design.inZone(0, board.player, target)) {
              var isForced = false;
