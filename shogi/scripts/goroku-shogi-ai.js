...
var materialTable = [0, 100, 300, 400, 400, 450, 1000, 1500, 1700, 2000, 600000];

var g_seeValues = [0, 1, 3, 4, 4, 4, 6, 6, 7, 7, 900, 0, 0, 0, 0, 0,
                   0, 1, 3, 4, 4, 4, 6, 6, 7, 7, 900, 0, 0, 0, 0, 0];
...
Dagaz.AI.ResetGame = function() {
  ...
  var pieceDeltas = [[], [], g_bishopDeltas, g_rookDeltas, g_rookDeltas, g_rookDeltas, g_bishopDeltas, g_rookDeltas, g_kingDeltas, g_kingDeltas, g_kingDeltas];
  ...
  for (var row = 0; row < (Dagaz.Model.HEIGHT << 4); row += 0x10) {
      for (var col = 0; col < Dagaz.Model.WIDTH; col++) {
           ...
           g_vectorDelta[index].pieceMask[Dagaz.AI.colorWhite >> Dagaz.AI.TYPE_SIZE] |= (1 << pieceGold) | (1 << piecePawnP) | (1 << pieceSilverP);
           index = square - (square - 15) + (Dagaz.AI.VECTORDELTA_SIZE >> 1);
           g_vectorDelta[index].pieceMask[Dagaz.AI.colorWhite >> Dagaz.AI.TYPE_SIZE] |= (1 << pieceGold) | (1 << piecePawnP) | (1 << pieceSilverP);
           index = square - (square + 17) + (Dagaz.AI.VECTORDELTA_SIZE >> 1);
           g_vectorDelta[index].pieceMask[0] |= (1 << pieceGold) | (1 << piecePawnP) | (1 << pieceSilverP);
           index = square - (square + 15) + (Dagaz.AI.VECTORDELTA_SIZE >> 1);
           g_vectorDelta[index].pieceMask[0] |= (1 << pieceGold) | (1 << piecePawnP) | (1 << pieceSilverP);
...
Dagaz.AI.MakeMove = function(move) {
    ...
    if (flags & moveflagPromotion) {
        ...
        if ((piece & Dagaz.AI.TYPE_MASK) == pieceSilver) newPiece |= pieceSilverP;
           ...
           else if ((piece & Dagaz.AI.TYPE_MASK) == piecePawn) newPiece |= piecePawnP;
...
Dagaz.AI.UnmakeMove = function(move) {
    ...
    if (flags & moveflagPromotion) {
        ...
        if ((Dagaz.AI.g_board[to] & Dagaz.AI.TYPE_MASK) == pieceSilverP) piece |= pieceSilver;
           ...
           else if ((Dagaz.AI.g_board[to] & Dagaz.AI.TYPE_MASK) == piecePawnP) piece |= piecePawn;
...
Dagaz.AI.See = function(move) {
    ...
    for (var pieceType = pieceSilver; pieceType <= pieceRookP; pieceType++) {
