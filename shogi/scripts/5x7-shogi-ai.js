...
Dagaz.AI.ResetGame = function() {
  ...
  var pieceDeltas = [[], [], [], g_bishopDeltas, g_rookDeltas, g_rookDeltas, g_rookDeltas, g_rookDeltas, g_rookDeltas, [], g_bishopDeltas, g_rookDeltas, g_kingDeltas, g_kingDeltas, g_kingDeltas];
  ...
  for (var row = 0; row < (Dagaz.Model.HEIGHT << 4); row += 0x10) {
      for (var col = 0; col < Dagaz.Model.WIDTH; col++) {
           ...
           g_vectorDelta[index].pieceMask[Dagaz.AI.colorWhite >> Dagaz.AI.TYPE_SIZE] |= (1 << pieceGold) | (1 << piecePawnP) | (1 << pieceSilverP) | (1 << pieceKnightP) | (1 << pieceLanceP);
...
function IsSquareAttackable(target, color) {
    ...
    gold = (color ? Dagaz.AI.colorWhite : Dagaz.AI.colorBlack) | pieceLanceP;
    if (Dagaz.AI.g_board[target - (inc - 1)] == gold) return true;
    if (Dagaz.AI.g_board[target - (inc + 1)] == gold) return true;
    if (Dagaz.AI.g_board[target - 16] == gold) return true;
    if (Dagaz.AI.g_board[target + 16] == gold) return true;
