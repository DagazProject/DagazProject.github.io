...
var toReserve = function(pos) {
  ...
  var y = pos >> 1;
  if (x) {
      y = Dagaz.Model.HEIGHT - y - 1;
      x += Dagaz.Model.WIDTH + 2;
  }
  return y * (Dagaz.Model.WIDTH + 2 * (Dagaz.AI.RESERVE_SIZE + 1)) + x;
}
  ...
  if (setup) {
      ...
      for (var i = 0; i < setup.length; i++) {
           ...
           if (c != '/') {
               ...
               if (pos >= (Dagaz.AI.RESERVE_SIZE * 2) * Dagaz.AI.RESERVE_LIMIT) break;
           }
      }
...
Dagaz.Model.getSetup = function(design, board) {
  ...
  str += '-';
  k = 0; c = 0;
  for (var pos = 0; pos < (Dagaz.AI.RESERVE_SIZE * 2) * Dagaz.AI.RESERVE_LIMIT; pos++) {
