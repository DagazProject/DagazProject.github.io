...
Dagaz.AI.g_board = new Array(256 * Dagaz.AI.PLANES); // Sentinel 0x80, pieces are in low 4 bits, 0x8 for color, 0x7 bits for piece type
...
Dagaz.AI.g_pieceIndex = new Array(256 * Dagaz.AI.PLANES);
...
function QSearch(alpha, beta, ply) {
    ...
    if (wasInCheck) {
        ...
        for (var i = 0; i < moves.length; i++) {
            var captured = Dagaz.AI.g_board[(moves[i] >> 12) & 0xFFF] & Dagaz.AI.TYPE_MASK;
            var pieceType = Dagaz.AI.g_board[moves[i] & 0xFFF] & Dagaz.AI.TYPE_MASK;
        ...
        if (++this.atMove == this.moveCount) {
            ...
            if (this.stage == 2) {
                ...
                for (var i = this.atMove; i < this.moveCount; i++) {
                    var captured = Dagaz.AI.g_board[(this.moves[i] >> 12) & 0xFFF] & Dagaz.AI.TYPE_MASK;
                    var pieceType = Dagaz.AI.g_board[this.moves[i] & 0xFFF] & Dagaz.AI.TYPE_MASK;
...
function AllCutNode(ply, depth, beta, allowNull) {
    ...
    for (;;) {
        ...
        if (value > realEval) {
            if (value >= beta) {
		var histTo = (currentMove >> 12) & 0xFFF;
		if (Dagaz.AI.g_board[histTo] == 0) {
                    if (Dagaz.AI.USE_HIST_TABLE) {
                        var histPiece = Dagaz.AI.g_board[currentMove & 0xFFF] & Dagaz.AI.PIECE_MASK;
...
//      if ((depth == 0) && (Dagaz.AI.FormatMove(currentMove) == 'f5g5')) g_trace = true;
        ...
        if (g_trace) {
            console.log('> ' + offset(depth) + Dagaz.AI.FormatMove(currentMove) + ' [' + value + ']');
        }
//      if ((depth == 0) && (Dagaz.AI.FormatMove(currentMove) == 'f5g5')) g_trace = false;
...
//  Dagaz.AI.g_board = new Array(256 * Dagaz.AI.PLANES);
//  Dagaz.AI.g_pieceIndex = new Array(256 * Dagaz.AI.PLANES);
    ...
    for (var i = 0; i < 32; i++) {
        Dagaz.AI.historyTable[i] = new Array(256 * Dagaz.AI.PLANES);
        for (var j = 0; j < 256 * Dagaz.AI.PLANES; j++)
            Dagaz.AI.historyTable[i][j] = 0;
    }

    var mt = new Dagaz.AI.MT(0x1badf00d);

    Dagaz.AI.g_zobristLow = new Array(256 * Dagaz.AI.PLANES);
    Dagaz.AI.g_zobristHigh = new Array(256 * Dagaz.AI.PLANES);
    for (var i = 0; i < 256 * Dagaz.AI.PLANES; i++) {
