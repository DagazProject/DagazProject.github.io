...
var checkPawn = function(design, board, player, pos, dir) {
  ...
  if (design.inZone(2, piece.player, p)) {
      var c = 0;
      for (var q = 49; q < design.positions.length; q++) {
...
Dagaz.Model.checkPositions = function(design, board, player, positions) {
  ...
  for (var i = 0; i < positions.length; i++) {
       ...
       if (checkDirection(design, board, player, pos, nw, [king], [bishop])) return true;
       if (checkDirection(design, board, player, pos, ne, [king], [bishop])) return true;
       if (checkDirection(design, board, player, pos, sw, [king], [bishop])) return true;
       if (checkDirection(design, board, player, pos, se, [king], [bishop])) return true;
